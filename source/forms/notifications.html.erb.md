...
**Note:** In this example the standard JavaScript `alert()` function is used to show a dialog generated by the web browser which respects the user’s  default settings for font-size, colors, and language. After confirming the message in the dialog window, most web browsers will resume the focus back on the “save” button. This functionality would need to be matched for custom dialog windows.
