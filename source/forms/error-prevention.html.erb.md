...
The best way to avoid errors in forms is to inform users about potential errors as soon as possible. HTML5 provides a range of validation options that gives browsers the possibility to notify users instantly about possible errors, usually in the language of the browser. It also prevents the user from submitting the form, so unnecessary back and forth to the server is avoided.
...
### Validating the format of form fields
{:.ex}
